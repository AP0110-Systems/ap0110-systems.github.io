...
date: "2026-06-01"
year: "2026"
description: "IETF Standards Track specification defining the QUERY method for HTTP — a safe, idempotent request that carries its query in the message body and returns the result of processing it."
author: "J. Reschke, J.M. Snell, M. Bishop"
sourceUrl: "https://datatracker.ietf.org/doc/rfc10008/"
pdfUrl: "/pdfs/rfc-10008-http-query.pdf"
kind: "public-domain"
...
Information about the current status of this document, any errata, and how to provide feedback on it may be obtained at [https://www.rfc-editor.org/info/rfc10008](https://www.rfc-editor.org/info/rfc10008).
...
This specification defines the HTTP QUERY request method as a means of making a safe, idempotent request ([Section 9.2](https://rfc-editor.org/rfc/rfc9110#section-9.2) of [HTTP]) that encloses a representation describing how the request is to be processed by the target resource.
...
-   size limits often are not known ahead of time because a request can pass through many uncoordinated systems (but note that [Section 4.1](https://rfc-editor.org/rfc/rfc9110#section-4.1) of [HTTP] recommends senders and recipients to support at least 8000 octets),
...
This document uses terminology defined in [Section 3](https://rfc-editor.org/rfc/rfc9110#section-3) of [HTTP].

Furthermore, it uses the terms _URI query parameter_ for parameters in the query component of a URI ([Section 4.2.2](https://rfc-editor.org/rfc/rfc9110#section-4.2.2) of [HTTP]) and _query content_ for the request content ([Section 6.4](https://rfc-editor.org/rfc/rfc9110#section-6.4) of [HTTP]) of a QUERY request.
...
Servers MUST fail the request if the Content-Type request field ([HTTP], [Section 8.3](https://rfc-editor.org/rfc/rfc9110#section-8.3)) is missing or is inconsistent with the request content.
...
QUERY requests are safe with regard to the target resource ([HTTP], [Section 9.2.1](https://rfc-editor.org/rfc/rfc9110#section-9.2.1)); that is, the client does not request or expect any change to the state of the target resource. This does not prevent the server from creating additional HTTP resources through which additional information can be retrieved (see Sections 2.3 and 2.4).

Furthermore, QUERY requests are idempotent ([HTTP], [Section 9.2.2](https://rfc-editor.org/rfc/rfc9110#section-9.2.2)); they can be retried or repeated when needed, for instance, after a connection failure.

As per [Section 15.3](https://rfc-editor.org/rfc/rfc9110#section-15.3) of [HTTP], a 2xx (Successful) response code signals that the request was successfully received, understood, and accepted.
...
The semantics of a QUERY request depend both on the request content and the associated metadata, such as the media type ([HTTP], [Section 8.3.1](https://rfc-editor.org/rfc/rfc9110#section-8.3.1)). In general, any problem with requests where content and metadata are inconsistent MUST be rejected with a 4xx (Client Error) response ([HTTP], [Section 15.5](https://rfc-editor.org/rfc/rfc9110#section-15.5)).
...
-   If the client requests a specific response media type using the Accept field ([HTTP], [Section 12.5.1](https://rfc-editor.org/rfc/rfc9110#section-12.5.1)) that is not supported by the resource, a status code of 406 (Not Acceptable) is appropriate.

### 2.2. Equivalent Resource

The _equivalent resource_ for any given QUERY request is a resource that responds to GET requests, represents that QUERY request and its target, and takes both message content and metadata into account ([Section 6](https://rfc-editor.org/rfc/rfc9110#section-6) of [HTTP]). In particular, this includes representation metadata ([Section 8](https://rfc-editor.org/rfc/rfc9110#section-8) of [HTTP]) such as the content's media type.
...
The term _equivalent resource_ is used as a means to define behavior for other HTTP aspects, such as selected representations. Servers can but do not have to assign URIs to these resources (see [Section 1.1](https://rfc-editor.org/rfc/rfc3986#section-1.1) of [URI]). If they do so, these resources will become accessible for GET requests.

### 2.3. Content-Location Response Field

A successful response (2xx, [Section 15.3](https://rfc-editor.org/rfc/rfc9110#section-15.3) of [HTTP]) can include a Content-Location header field containing an identifier for a resource corresponding to the results of the operation; see [Section 8.7](https://rfc-editor.org/rfc/rfc9110#section-8.7) of [HTTP] for details. This represents a claim from the server that a client can send a GET request for the indicated URI to retrieve the results of the query operation just performed. The indicated resource might be temporary.

See Appendix A.4.1 for an example.

### 2.4. Location Response Field

A server can assign a URI to the equivalent resource (Section 2.2) of a QUERY request. If the server does so, the URI of that resource can be included in the Location header field of the 2xx response (see [Section 10.2.2](https://rfc-editor.org/rfc/rfc9110#section-10.2.2) of [HTTP]). This represents a claim that a client can send a GET request to the indicated URI to repeat the query operation just performed without resending the query content. This resource's URI might be temporary; if a future request fails, the client can retry using the original QUERY request target and the previously submitted content.

See Appendix A.4.2 for an example.

### 2.5. Redirection

In some cases, the server may choose to respond indirectly to the QUERY request by redirecting the user agent to a different URI (see [Section 15.4](https://rfc-editor.org/rfc/rfc9110#section-15.4) of [HTTP]).

A response with either status codes 301 (Moved Permanently, [HTTP], [Section 15.4.2](https://rfc-editor.org/rfc/rfc9110#section-15.4.2)) or 308 (Permanent Redirect, [HTTP], [Section 15.4.9](https://rfc-editor.org/rfc/rfc9110#section-15.4.9)) indicates that the target resource has permanently moved to a different URI referenced by the Location response field ([HTTP], [Section 10.2.2](https://rfc-editor.org/rfc/rfc9110#section-10.2.2)). Likewise, a response with either status codes 302 (Found, [HTTP], [Section 15.4.3](https://rfc-editor.org/rfc/rfc9110#section-15.4.3)) or 307 (Temporary Redirect, [HTTP], [Section 15.4.8](https://rfc-editor.org/rfc/rfc9110#section-15.4.8)) indicates that the target resource has temporarily moved. In all four cases, the server is suggesting that the user agent can accomplish its original QUERY request by sending a similar QUERY request to the new target URI referenced by the Location.
...
A response to QUERY with the status code 303 (See Other, [Section 15.4.4](https://rfc-editor.org/rfc/rfc9110#section-15.4.4) of [HTTP]) indicates that the original query can be accomplished via a normal retrieval request on the URI referenced by the Location response field ([HTTP], [Section 10.2.2](https://rfc-editor.org/rfc/rfc9110#section-10.2.2)). For HTTP, this means sending a GET request to the new target URI, as illustrated by the example in Appendix A.4.3.

### 2.6. Conditional Requests

The selected representation ([Section 3.2](https://rfc-editor.org/rfc/rfc9110#section-3.2) of [HTTP]) of a QUERY request is the same as for a GET request to the equivalent resource (Section 2.2) of that QUERY request.

A conditional QUERY requests that the selected representation (i.e., the query results after any content negotiation) be returned in the response only under the circumstances described by the conditional header field(s), as defined in [Section 13](https://rfc-editor.org/rfc/rfc9110#section-13) of [HTTP].

See Appendix A.5 for examples.

### 2.7. Caching

The response to a QUERY method is cacheable; a cache MAY use it to satisfy subsequent QUERY requests as per [Section 4](https://rfc-editor.org/rfc/rfc9111#section-4) of [HTTP-CACHING].

The cache key for a QUERY request ([Section 2](https://rfc-editor.org/rfc/rfc9111#section-2) of [HTTP-CACHING]) MUST incorporate the request content ([Section 6](https://rfc-editor.org/rfc/rfc9111#section-6) of [HTTP-CACHING]) and related metadata ([Section 8](https://rfc-editor.org/rfc/rfc9110#section-8) of [HTTP]).
...
-   removing content encoding(s) ([Section 8.4](https://rfc-editor.org/rfc/rfc9110#section-8.4) of [HTTP]).
-   normalizing based upon knowledge of format conventions, as indicated by any media subtype suffix in the request's Content-Type field (e.g., "+json"; see [Section 4.2.8](https://rfc-editor.org/rfc/rfc6838#section-4.2.8) of [RFC6838]).
...
Clients can indicate, using the "no-transform" cache directive ([Section 5.2.1.6](https://rfc-editor.org/rfc/rfc9111#section-5.2.1.6) of [HTTP-CACHING]) that they wish that no such transformation happens (but note that this directive is just advisory).
...
The semantics of Range Requests for QUERY are identical to those for GET, as defined in [Section 14](https://rfc-editor.org/rfc/rfc9110#section-14) of [HTTP]. Byte Range Requests (the only range unit defined at the time of writing), however, offer little value for the results of a QUERY request.
...
Although the syntax for this field appears to be similar to other fields, such as "Accept" ([Section 12.5.1](https://rfc-editor.org/rfc/rfc9110#section-12.5.1) of [HTTP]), it is a Structured Field and thus MUST be processed as specified in [Section 4](https://rfc-editor.org/rfc/rfc9651#section-4) of [STRUCTURED-FIELDS].
...
IANA has added the QUERY method to the "Hypertext Transfer Protocol (HTTP) Method Registry" at <[http://www.iana.org/assignments/http-methods](http://www.iana.org/assignments/http-methods)> (see [Section 16.3.1](https://rfc-editor.org/rfc/rfc9110#section-16.3.1) of [HTTP]).
...
Section 2 of RFC 10008

### 5.2. Registration of the Accept-Query Field

IANA has added the Accept-Query field to the "Hypertext Transfer Protocol (HTTP) Field Name Registry" at <[https://www.iana.org/assignments/http-fields](https://www.iana.org/assignments/http-fields)> (see [Section 16.1.1](https://rfc-editor.org/rfc/rfc9110#section-16.1.1) of [HTTP]).
...
A simple way to discover support for QUERY is provided by the OPTIONS ([Section 9.3.7](https://rfc-editor.org/rfc/rfc9110#section-9.3.7) of [HTTP]) method:
...
The Allow response field ([Section 10.2.1](https://rfc-editor.org/rfc/rfc9110#section-10.2.1) of [HTTP]) denotes the set of supported methods on the specified resource.
...
would result in a 304 (Not Modified) response ([Section 15.4.5](https://rfc-editor.org/rfc/rfc9110#section-15.4.5) of [HTTP]).

#### A.4.3. Indirect Responses

Servers can send "indirect" responses (Section 2.5) using the status code 303 (See Other, [Section 15.4.4](https://rfc-editor.org/rfc/rfc9110#section-15.4.4) of [HTTP]).
...
The "Hypertext Transfer Protocol (HTTP) Method Registry" (<[http://www.iana.org/assignments/http-methods](http://www.iana.org/assignments/http-methods)>) already contains three other methods with the properties "safe" and "idempotent": "PROPFIND" [RFC4918], "REPORT" [RFC3253], and "SEARCH" [RFC5323].
...
Email: [asbjorn@ulsberg.no](mailto:asbjorn@ulsberg.no)

URI: [https://asbjor.nu/](https://asbjor.nu/)
