...
// Lazy so sites with showSearch=false (ap0110.com) never ship the search bundle.
...
function SocialLink({ href, label, children }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="w-[40px] h-[40px] flex opacity-50 hover:opacity-100 transition-opacity">
      {children}
    </a>
  );
}
...
export default function Header({
  navItems = [
    ...
    { type: 'link', label: 'Lab', href: '/lab' },
  ],
  theme = 'dark',           // 'light' | 'dark'
  logoText = 'AP0110',
  pathname = '/',
  cta = { label: 'Start a project', href: '/contact' },   // null → no CTA button
  showSearch = false,       // Cmd/Ctrl-K wiki search (ap0110.org)
}) {
  ...
  // Native <dialog> drives the mobile menu: top-layer, focus trap, Esc, and
  // focus restore come from the platform. onClose keeps state in sync.
  useEffect(() => {
  ...
  // Cmd/Ctrl-K opens the wiki search from any page (only where search is enabled).
  useEffect(() => {
  ...
  // Nav-bar: translucent + blur; hairline border fades in once scrolled.
  const headerBg = isDark
    ? (scrolled
        ? 'bg-surface-dark/80 backdrop-blur-md border-b border-surface-dark-muted transition-colors'
        : 'bg-surface-dark/80 backdrop-blur-md border-b border-transparent transition-colors')
    : (scrolled
        ? 'bg-surface/80 backdrop-blur-md border-b border-surface-muted transition-colors'
        : 'bg-surface/80 backdrop-blur-md border-b border-transparent transition-colors');

  return (
    <>
      <header
        data-site-header
        className={`fixed left-0 right-0 w-full z-50 pointer-events-none h-[60px] ${headerBg}`}
      >
        <div className="relative w-full h-full">
          {/* Logo: icon dominates (~36% of bar height, Cloudflare ratio), wordmark
              secondary at text-sm; full-height hit target. */}
          <a
            href="/"
            aria-label="AP0110 home"
            className="absolute top-0 left-0 h-full font-mono font-bold w-fit flex items-center px-4 md:px-6 gap-3 pointer-events-auto cursor-pointer hover:opacity-80 transition-opacity"
          >
            <svg
              width="22"
              height="22"
              viewBox="0, 0, 400, 400"
              fill="currentColor"
              className={`${isDark ? 'text-gray-100' : 'text-black'} inline shrink-0`}
            >
            ...
            {navText(logoText, isHomePage, 20, 'hidden sm:inline text-sm')}
          </a>
          ...
          <nav className={`hidden md:flex absolute top-0 right-0 h-[60px] items-center font-medium pointer-events-auto ${isDark ? 'text-on-dark' : 'text-on-surface'}`}>
            {navItems.map((item, index) => {
              ...
              return (
                <a
                  ...
                  className={`text-sm px-4 md:px-5 h-full flex items-center border-b-2 transition-colors hover:text-ap-cyan ${isActive ? 'border-ap-cyan' : 'border-transparent'}`}
                >
                  {navText(item.label, isHomePage, NAV_TYPING_SPEED, 'text-sm')}
                </a>
              );
            })}
            {showSearch && (
              <button
                ...
                className="text-sm px-4 md:px-5 h-full flex items-center gap-2 border-b-2 border-transparent transition-colors hover:text-ap-cyan cursor-pointer"
              ...
              >
                ...
                {isMac !== null && (
                  <kbd className={`hidden lg:inline font-mono text-xs leading-none px-1.5 py-1 rounded-sm border ${isDark ? 'border-surface-dark-muted text-on-dark-mid' : 'border-surface-muted text-on-surface-low'}`}>
                    {isMac ? '⌘K' : 'Ctrl K'}
                  </kbd>
                )}
              </button>
            )}
            {/* Persistent primary CTA (Cloudflare-style "Sign up" analog) */}
            {cta && (
              <a href={cta.href} className="ap-button primary size-sm ml-3 mr-4 md:mr-6">
                {cta.label}
              </a>
            )}
          </nav>
      ...
      {/* Mobile menu — native dialog: top-layer, focus-trapped, Esc closes.
          pointer-events-auto is required (the header parent is pointer-events-none). */}
      <dialog
        ref={menuRef}
        id="mobile-menu"
        data-chrome-dialog
        ...
        className={`m-0 p-0 w-full h-full max-w-none max-h-none bg-transparent pointer-events-auto ${isDark ? 'backdrop:bg-black/75' : 'backdrop:bg-white/75'} backdrop:backdrop-blur-lg`}
      >
        <div className="relative w-full h-full">
          <nav
            aria-label="Site menu"
            className="absolute top-[40px] left-0 text-base font-medium px-[1em] md:px-[3.5em] pt-4 w-fit"
          >
                ...
                return (
                  <li key={index}>
                    <a
                      href={item.href}
                      aria-current={isActive ? 'page' : undefined}
                      className={`${isActive ? 'text-ap-cyan' : isDark ? 'text-gray-200' : 'text-black'} hover:text-ap-cyan transition-colors block w-fit`}
                      onClick={() => setMenuOpen(false)}
                    >
                      {menuOpen && <TypingText text={item.label} typingSpeed={40} initialDelay={0} className="inline" />}
                    </a>
                  </li>
                );
              })}
              {showSearch && (
                <li>
                  <button
                    ...
                    className={`${isDark ? 'text-gray-200' : 'text-black'} hover:text-ap-cyan transition-colors block w-fit cursor-pointer`}
                  >
                    {menuOpen && <TypingText text="Search" typingSpeed={40} initialDelay={0} className="inline" />}
                  </button>
                </li>
              )}
            </ul>
            {cta && (
              <a
                href={cta.href}
                className="ap-button primary size-md mt-6 inline-flex"
                onClick={() => setMenuOpen(false)}
              >
                {cta.label}
              </a>
          ...
          <div className={`absolute bottom-0 right-0 w-fit h-fit flex gap-4 flex-col md:flex-row px-2 md:px-6 py-2 md:p-0 justify-end align-bottom ${isDark ? 'text-gray-200' : 'text-black'}`}>
            <SocialLink href="https://www.linkedin.com/company/ap0110" label="AP0110 on LinkedIn">
            ...
            <SocialLink href="https://x.com/AP0110_net" label="AP0110 on X">
            ...
            <a
              ...
              aria-label="Email AP0110"
              className="w-[40px] h-[40px] flex opacity-50 hover:opacity-100 transition-opacity"
            >
