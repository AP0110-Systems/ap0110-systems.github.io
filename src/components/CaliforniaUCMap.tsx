...
interface CaliforniaUCMapProps {
...
const CaliforniaUCMap: React.FC<CaliforniaUCMapProps> = ({ 
  ...
  const [countyData, setCountyData] = React.useState<CountyData | null>(null)
  ...
  useEffect(() => {
    ...
    const allPoints: [number, number][] = UC_CAMPUSES.map((campus) => [campus.lat, campus.lng])
    countyData.features.forEach((feature) => {
      const [lng, lat] = feature.geometry.coordinates
