...
import CaliforniaUCMap from '@/components/CaliforniaUCMap'
...
export default function CalComputePage({ faq = [] }) {
  return (
    <div className="min-h-screen uc-sans" style={{ backgroundColor: '#F8FAFC', color: '#0F172A' }}>
      ...
      {/* ── FAQ (visible, mirrors the FAQPage schema in calcompute.astro) ── */}
      {faq.length > 0 && (
        <section style={{ backgroundColor: '#FFFFFF', borderTop: '1px solid #E2E8F0' }} className="px-6 py-16">
          <div className="max-w-3xl mx-auto">
            <h2 style={{ fontSize: '32px', fontWeight: 700, lineHeight: 1.2, letterSpacing: '-0.02em', color: '#0A0F1C', marginBottom: '32px' }}>
              Frequently asked questions
            </h2>
            <div className="space-y-8">
              {faq.map((f) => (
                <div key={f.q}>
                  <h3 style={{ fontSize: '20px', fontWeight: 600, color: '#0A0F1C', marginBottom: '8px' }}>{f.q}</h3>
                  <p style={{ fontSize: '17px', lineHeight: 1.7, color: '#4B5563' }}>{f.a}</p>
                </div>
              ))}
            </div>
          </div>
